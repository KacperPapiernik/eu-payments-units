...
async def process(req: PaymentRequest) -> dict:
    ...
    if weekday >= 5 or hour < 7 or hour >= 18:
        ...
        tx = await save_transaction(
            ...
            message=f"TARGET transfer scheduled for {scheduled_for.strftime('%Y-%m-%d %H:%M')} CET"
        )
    else:
        tx = await save_transaction(
            sender_iban=req.sender_iban,
            ...
            description=req.description,
            status="RECEIVED",
            message="TARGET transfer received, processing via RTGS"
        )
